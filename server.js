...
app.get('/api/records', async (req, res) => {
  try {
    ...
    if (search)   q.$or = [
      ...
      { mgrName:    new RegExp(search, 'i') },
      { defectCategory: new RegExp(search, 'i') },
      { defectType: new RegExp(search, 'i') },
...
app.post('/api/records', upload.single('photo'), async (req, res) => {
  try {
    const b = req.body;
    const qtyDefect = Number(b.qtyDefect) || 0;
    const qtyNG     = Number(b.qtyNG)     || 0;
    ...
    const rec = new Record({
      ...
      qtyDefect,
      ncrRef:     b.ncrRef     || '',
      defectCategory: b.defectCategory || '',
      defectType:     b.defectType     || '',
      qtyNG,
      ...
      empId:       b.empId       || '',
      empDept:     b.empDept     || '',
      empPosition: b.empPosition || '',
      empDecision: empDec,
      empRemark:   b.empRemark   || '',
      mgrName:     b.mgrName     || '',
      mgrPosition: b.mgrPosition || '',
      mgrDecision: mgrDec,
      mgrRemark:   b.mgrRemark   || '',
      decisionDate: b.decisionDate || '',
      problemDate:  b.problemDate  || '',
      alignment,
    });
    await rec.save();
...
// PATCH edit (admin)
app.patch('/api/records/:id', upload.single('photo'), async (req, res) => {
  ...
  try {
    ...
    fields.forEach(f => { if (b[f] !== undefined) rec[f] = b[f]; });
    if (req.file) {
      // delete old photo
      if (rec.photoPath) {
        const old = path.join(__dirname, 'public', rec.photoPath);
        if (fs.existsSync(old)) fs.unlinkSync(old);
      }
      rec.photoPath = `/uploads/${req.file.filename}`;
    }
    const empDec = rec.empDecision;
    const mgrDec = rec.mgrDecision;
    rec.alignment = !empDec || !mgrDec ? '— รอการตัดสินใจ'
                  : empDec === mgrDec   ? `✅ ตรงกัน — ${mgrDec}`
                  : `⚠ ไม่ตรงกัน | พนักงาน: ${empDec} | ผู้บริหาร: ${mgrDec}`;
    await rec.save();
    res.json({ ok: true });
...
// DELETE (admin)
app.delete('/api/records/:id', express.json(), async (req, res) => {
  if (!checkAdmin(req, res)) return;
  try {
    const rec = await Record.findOne({ id: req.params.id });
    if (!rec) return res.status(404).json({ error: 'Not found' });
    if (rec.photoPath) {
      const p = path.join(__dirname, 'public', rec.photoPath);
